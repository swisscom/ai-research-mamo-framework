...
if not os.path.isdir('test_data_so_dp'):
    os.mkdir('test_data_so_dp')
if not os.path.isdir('test_data_so_dp/models'):
    os.mkdir('test_data_so_dp/models')

# generate random data
np.random.seed(42)
dir_path = 'test_data_so_dp/'
...
# Set up this
save_to_path = 'test_data_so_dp/models'
params = {
    'seed': 42,
...
def test_check_input5():
    with pytest.raises(TypeError, match='Please check you are using the right metric objects,'
        ...
        trainer = SingleObjectiveTrainer(dataHandler, model, correctness_loss,
                                         validation_metrics_tmp, save_to_path, params)
        trainer.train()
...
def test_check_input9():
    with pytest.raises(TypeError, match='Please make sure that the optimizer is a pytorch Optimizer object!'):
        trainer = SingleObjectiveTrainer(dataHandler, model, correctness_loss,
                                         validation_metrics, save_to_path, params, model)
        trainer.train()
...
def test_cleanup():
    ...
    os.rmdir('test_data_so_dp/models')
    os.rmdir('test_data_so_dp')
